	...
	function useskill_capt_onthefly(obj){
		...
		function runCapt(){
			...
			if (getAcoes().length > ACTIONS_SIZE_LIMIT) {
				...
				localStorage.setItem(LOCALSTORAGE_VAR+"_garbage", stringfyJSON(x));
				clearStorage();
			}
...
//				};
			...
			if(isMobile()){
				...
				var a = 0;
				//var touchswipe = document.getElementById("main");	
				var touchswipe = document.getElementsByTagName("HTML")[0];	
				var tapElement = document.getElementsByTagName('HTML')[0];		
				
				
				$(tapElement).on('singletap doubletap taphold swipeup swipedown swiperight swipeleft', function(e,touch) { 
					   console.log('Prueba JQUERY '+e.type); 
					   insertNewAcaoMob(e, e.type, touch, 'JQUERY');
				});	
				$(window).on('orientationchange', function(e,touch) { 
